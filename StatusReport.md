...
Analyze relationships between attributes and chart rank and work on an automated end-to-end workflow while making sure all the content is reproducible enough. Budget time to write up the analysis of the data life cycle and ethical data handling (licenses, copyright, etc).
...
### Veer
For my portion, I conducted substantial data cleaning on the Billboard Hot 100 dataset, including removing duplicates, standardizing artist and title formats, and resolving inconsistencies that would have affected downstream merging with Spotify’s API data. pivoted to what variables and attributes to use when making the decision to lower the attributes to use for our analysis. I am also making sure that the team is hitting all steps on their deadlines. In addition to data preparation, I contributed to the core coding logic, including early scripts for matching Billboard songs with Spotify tracks and structuring our unified dataset. I also helped coordinate task progress within the group, making sure Om and I stayed aligned with our updated timeline and met key deadlines. Overall, Om and I have worked and communicated well about next steps and are excited for the coming weeks with a better plan for our project and data.
